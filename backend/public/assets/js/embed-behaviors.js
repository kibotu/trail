/**
 * Shared embed behaviors for embeddable pages.
 *
 * Read-only widgets (user embed + collection embed) share these behaviors:
 * auto-resize height reporting, click overrides (open links in new tabs,
 * embed-aware share modal). card-template.js uses window.location for
 * navigation and window.location.origin for share URLs, which don't work
 * correctly inside a cross-origin iframe — these intercept at the container
 * level before card-template.js handlers run.
 */

/**
 * Initialize embed behaviors for a container.
 *
 * @param {HTMLElement} entriesContainer - The entries container element.
 * @param {string} baseUrl - The base URL of the Trail app.
 * @returns {{ postHeight: Function }} Post height to parent for auto-resize.
 */
function initEmbedBehaviors(entriesContainer, baseUrl) {
    'use strict';
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { initEmbedBehaviors };
}
